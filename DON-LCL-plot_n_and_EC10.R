...
n_df <- fitparms_df[,nnames]
names(n_df) <- as.character(cells)
write.csv(t(apply(n_df,2,quantile,prob=c(0.025,0.5,0.975))),
          file="Table-DON_n_CellLine_summary.csv")
n_df.long <- pivot_longer(n_df,cols = 1:ncol(n_df))
pn <- ggplot(n_df.long) + 
  geom_boxplot(aes(x=reorder(name,value,median),y=value),
               outlier.shape = NA) +
  coord_flip() + scale_y_log10() + ylab("n") + xlab("Cell Line") +
  theme(axis.title = element_text(size = 15))
print(pn)
ggsave("Figure-DON_n.pdf", plot=pn,width=8,height=10)
